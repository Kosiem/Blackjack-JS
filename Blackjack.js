...
function HandStartDealer(){
    ...
    console.log(random);
    card.setAttribute("src", cardDeck[random].toString() + ".png")
    card.setAttribute("class", "cardDealer");
    blank.setAttribute("src" , "back_of_card.png")
...
function CountPoints(){
    ...
    StandBtn.disabled = true;
        ...
        PlayerPoints.innerHTML = "You win, blackjack";
        CheckBetWin();
        HitBtn.disabled = true;
        StandBtn.disabled = true;
        ResetBtn.disabled = false;
    ...
    if(StandStatus == false){
       ...
       let getValueD = cardNameD.split("_");
...
function ChangeBlankCard(){
    ...
    blank.setAttribute("src",cardDeck[random].toString() + ".png")
...
function CheckBetWin(doubleDown){
    if(BetStatus == false){
        if(endGame.innerHTML == ""){
            ...
            const newTokens = parseInt(localStorage.getItem("tokens"));
            currenTokens = newTokens - currentBet;
            const sum = currenTokens + (currentBet * 2)
            ...
            const newTokens = parseInt(localStorage.getItem("tokens"));
            currenTokens = newTokens - currentBet;
            const sum = currenTokens + (currentBet * 2.5)
            localStorage.setItem("tokens", parseInt(sum))
            if(doubleDown == true){
                const currentBet = parseInt(BetInput.value * 2);
                const newTokens = parseInt(localStorage.getItem("tokens"));
